...
export function McqBnEnQuiz({ words: initialWords, onAnswer }: McqBnEnQuizProps) {
    ...
    const generateOptions = useCallback((correctWord: Word) => {
        ...
        const newOptions = shuffle([correctWord.word, ...incorrectOptions]);
    ...
    const getButtonVariant = (option: string) => {
        ...
        if (option === currentWord.word) return 'default'; // Correct answer
        if (option === selectedAnswer) return 'destructive'; // Incorrectly selected by user
        return 'outline';
    };
     const getButtonClass = (option: string) => {
        if (!selectedAnswer) return '';
        if (option === currentWord.word) return 'bg-green-500 hover:bg-green-600'; // Correct answer
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
            <div className="text-center mb-8">
                <p className="text-lg text-muted-foreground">Which word means:</p>
                <h2 className="text-4xl font-bold my-2">{currentWord.meaning}</h2>
            </div>
